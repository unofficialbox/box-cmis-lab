...
- Prefer `@unofficialbox/box-open-elements` components before custom UI; use `box-tree-grid` for the folder browser with columns for CMIS base type, Object ID, and last modified; keep the name/first column sticky so deep indentation does not overlap other columns; follow the community design in that repo’s `docs-site/DESIGN.md`.
...
- On page refresh, reuse still-valid auth sessions (refresh expired access tokens when possible) instead of forcing reconnect.
...
- OAuth credentials persist in `sessionStorage` for same-tab refresh; expired access tokens are refreshed via refresh token when possible.
- README/banner branding follows the unofficialbox pattern (see `box-gantry`); logo and banner SVG assets live under `assets/`.
